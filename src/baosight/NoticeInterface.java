...
import sgddbasedataserver.*;
import sgddbasedataserver.BasicDataImplService;
...
//					result = ResponseMessage.getInstance("310", "服务器内部异常", "-1", "失败");
        ...
        logobj.accumulate("method", "NoticeInterface");
        logobj.accumulate("methodparam", pstatus + "-" + verb + "-" + noun + "-"+pkname + "=" + pkval);
        logobj.accumulate("errtype", "NoticeInterface");
        ...
        logobj.accumulate("result", result);
        //写log
	...
	try {
		...
		logobj.accumulate("errtype", "huaInterface"+sysflag);
		//写log
		utils.createLog(logobj.toString());
